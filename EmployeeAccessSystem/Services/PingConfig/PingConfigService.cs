using EmployeeAccessSystem.Models;
using EmployeeAccessSystem.Repositories;
using System.Globalization;

namespace EmployeeAccessSystem.Services
{
    public class PingConfigService : IPingConfigService
    {
        private readonly IPingConfigRepository _repo;

        public PingConfigService(IPingConfigRepository repo)
        {
            _repo = repo;
        }

        public async Task<IEnumerable<PingConfig>> GetAllAsync()
        {
            return await _repo.GetAllAsync();
        }

        public async Task<PingConfig?> GetByIdAsync(int id)
        {
            return await _repo.GetByIdAsync(id);
        }

        public async Task<PingConfig?> GetExistingAsync(int productId, int pingProductId, int itemId, DateTime date)
        {
            return await _repo.GetExistingAsync(productId, pingProductId, itemId, date);
        }

        public async Task<string> AddAsync(PingConfig model, string? currentUser)
        {
            try
            {
                string validationMessage = ValidateModel(model, false);
                if (!string.IsNullOrWhiteSpace(validationMessage))
                {
                    return "error|" + validationMessage;
                }

                model.EntryDate = model.EntryDate.Date;

                PingConfig? existing = await _repo.GetExistingAsync(
                    model.ProductId,
                    model.PingProductId,
                    model.PingProductItemId,
                    model.EntryDate
                );

                if (existing != null)
                {
                    return "error|Configuration already exists for the selected date.";
                }

                model.CreatedDate = DateTime.Now;
                model.CreatedBy = currentUser;

                int result = await _repo.AddAsync(model);

                if (result > 0)
                {
                    return "success|Saved successfully.";
                }

                return "error|Data could not be saved.";
            }
            catch
            {
                return "error|Database error occurred.";
            }
        }

        public async Task<string> UpdateAsync(PingConfig model, string? currentUser)
        {
            try
            {
                string validationMessage = ValidateModel(model, true);
                if (!string.IsNullOrWhiteSpace(validationMessage))
                {
                    return "error|" + validationMessage;
                }

                model.EntryDate = model.EntryDate.Date;

                PingConfig? existing = await _repo.GetExistingAsync(
                    model.ProductId,
                    model.PingProductId,
                    model.PingProductItemId,
                    model.EntryDate
                );

                if (existing != null && existing.PingConfigId != model.PingConfigId)
                {
                    return "error|Configuration already exists for the selected date.";
                }

                model.ModifiedDate = DateTime.Now;
                model.ModifiedBy = currentUser;

                int result = await _repo.UpdateAsync(model);

                if (result > 0)
                {
                    return "success|Updated successfully.";
                }

                return "error|Data could not be updated.";
            }
            catch
            {
                return "error|Database error occurred.";
            }
        }

        public async Task<string> DeleteAsync(int id, string? currentUser)
        {
            try
            {
                if (id <= 0)
                {
                    return "error|Invalid configuration id.";
                }

                PingConfig? existing = await _repo.GetByIdAsync(id);

                if (existing == null)
                {
                    return "error|Configuration not found.";
                }

                int result = await _repo.DeleteAsync(id, DateTime.Now, currentUser);

                if (result > 0)
                {
                    return "success|Deleted successfully.";
                }

                return "error|Delete failed.";
            }
            catch
            {
                return "error|Database error occurred.";
            }
        }

        public async Task<string> ToggleAsync(int id)
        {
            try
            {
                if (id <= 0)
                {
                    return "error|Invalid configuration id.";
                }

                PingConfig? existing = await _repo.GetByIdAsync(id);

                if (existing == null)
                {
                    return "error|Configuration not found.";
                }

                int result = await _repo.ToggleAsync(id);

                if (result > 0)
                {
                    return "success|Status updated successfully.";
                }

                return "error|Status could not be updated.";
            }
            catch
            {
                return "error|Database error occurred.";
            }
        }

        private string ValidateModel(PingConfig model, bool isUpdate)
        {
            if (isUpdate && model.PingConfigId <= 0)
            {
                return "Invalid configuration id.";
            }

            if (model.ProductId <= 0)
            {
                return "Please select Product.";
            }

            if (model.PingProductId <= 0)
            {
                return "Please select Ping Product.";
            }

            if (model.PingProductItemId <= 0)
            {
                return "Please select Ping Product Item.";
            }

            if (string.IsNullOrWhiteSpace(model.EntryMode))
            {
                return "Please select Save Option.";
            }

            if (model.EntryMode != "Value" && model.EntryMode != "Checkbox")
            {
                return "Invalid Save Option.";
            }

            if (model.EntryMode == "Value")
            {
                if (string.IsNullOrWhiteSpace(model.ConfigValue))
                {
                    return "Please enter value.";
                }

                model.ConfigValue = model.ConfigValue.Trim();
                model.IsChecked = false;

                decimal number;
                bool isValidNumber = decimal.TryParse(
                    model.ConfigValue,
                    NumberStyles.AllowDecimalPoint,
                    CultureInfo.InvariantCulture,
                    out number
                );

                if (!isValidNumber)
                {
                    return "Value must be numeric only.";
                }

                if (number < 0)
                {
                    return "Value cannot be less than 0.";
                }

                if (number > 100)
                {
                    return "Value cannot be greater than 100.";
                }

                model.ConfigValue = number.ToString("0.##", CultureInfo.InvariantCulture);
            }
            else if (model.EntryMode == "Checkbox")
            {
                model.ConfigValue = null;
            }

            return string.Empty;
        }
    }
}